<template>
	<canvas class="box" :style="{width: width + 'px', height: height + 'px', 'z-index': zIndex}" :canvas-id="canvasId"
...
<script>
	...
	*/
	const minBrowserRefreshTime = window && (window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame ||
			...
			fireworksDraw() {
				...
				for (; particleCount--;) {
					...
					ribbon.push({ //菜单位置初始化
						...
						color: (i = (n.color + "").replace(/[^0-9a-f]/gi, ""), i.length < 6 && (i = i[0] + i[0] + i[1] + i[1] + i[2] +
							i[2]), { //生成随机颜色
				...
				minBrowserRefreshTime(function drawRibbon() {
					...
					fireCanvasBox.draw(),
						...
						ribbon = ribbon.filter((e) => {
							...
							// 开始画图
							fireCanvasBox.fillStyle = "rgba(" + e.color.r + ", " + e.color.g + ", " + e.color.b + ", " + (1 - (e.tick++) /
									e.totalTicks) + ")",
